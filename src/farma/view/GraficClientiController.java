...
public class GraficClientiController implements Initializable, DBConstants {

    @FXML
    private Label valoriGraficLB;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        String VANZARI_LB = "Iesiri pe ultimul trimestru";
        valoriTitluLB.setText(VANZARI_LB);
        arrSerii = new XYChart.Series[20];

        graficAxaX.setLabel("Parteneri");
        graficAxaY.setLabel("Valoare (lei)");
        grafic.setTitle(VANZARI_LB);
        grafic.setLegendVisible(false);

        try {
            adaugaValoriLaGrafic(SELECT_IESIRI_T);
